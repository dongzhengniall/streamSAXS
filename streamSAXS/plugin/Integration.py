...
class Integrate2D(ProcessingFunction):
    ...
    def __init__(self):
        super().__init__()
        self._params_dict["npt_radial"] = {"type": "int", "value": 1000, "text": "Number of radial points"}
        self._params_dict["radial_unit"] = {"type": "enum", "value": Integrate2D_Unit.unit1,
                                            "text": "Radial Range Unit"}
    ...
    def run_function(self, data, objectset):
        ...
        result2d = bf.integrate2D(objectset['integrator'], data['image'], npt_rad=self.get_param_value("npt_radial"),
                                  mask=mask, unit=radialUnit, radial_range=self.get_param_value("radial_range"),
        ...
        return {'data': {'x': result2d.radial, 'y': result2d.azimuthal, 'z': result2d.intensity},
                'label': {'xlabel': self.get_param_value("radial_unit"), 'ylabel': 'chi (degree)'},
                'plot': {'data': {'x': result2d.radial, 'y': result2d.azimuthal, 'z': result2d.intensity},
                         'type': '2DXY',
                         'label': {'xlabel': self.get_param_value("radial_unit").value, 'ylabel': 'chi (degree)'}}
                }
...
# ------------------------------------------------------------------------------
class IntegrateAzimuthal(ProcessingFunction): # with error
    ...
    def __init__(self):
        ...
        self._params_dict["polarization_factor"] = {"type": "float", "value": 0.99, "text": "Polarization Factor",
                                                    "tip": "-1 (vertical) ~ +1 (horizontal). Default[None] is no correction, 0 for circular polarization or random."}
        self._params_dict['error'] = {'type': 'bool', 'value': False, 'text': 'Intensity uncertainty',
                                      'tip': "If selected, Intensity uncertainty will be given ."}

    def run_function(self, data, objectset):
        self.param_validation()
        # self.isData2D()

        if self.get_param_value("out_unit") == IntegrateAzimuthal_Unit.unit5:
            unit = 'r_mm'
        if self.get_param_value("out_unit") == IntegrateAzimuthal_Unit.unit4:
            unit = '2th_rad'
        elif self.get_param_value("out_unit") == IntegrateAzimuthal_Unit.unit3:
            unit = '2th_deg'
        elif self.get_param_value("out_unit") == IntegrateAzimuthal_Unit.unit2:
            unit = 'q_A^-1'
        elif self.get_param_value("out_unit") == IntegrateAzimuthal_Unit.unit1:
            unit = 'q_nm^-1'
        ...
        error_model = None
        if self.get_param_value("error") is True:
            error_model = 'poisson'  # azimuthal

        result1d = bf.integrateAzimuthal(integrator=objectset['integrator'], data=data['image'], mask=mask,
        ...
        data['x'] ,data['y'] = result1d.radial,result1d.intensity
        if self.get_param_value("error") is True:
            data['error']=result1d.sigma
        return {'data': data,
                'label': {'xlabel': self.get_param_value("out_unit").value, 'ylabel': 'Intensity'},
...
class IntegrateRadial(ProcessingFunction):
    ...
    def __init__(self):
        ...
        self._params_dict["npt_radial"] = {"type": "int", "value": None, "text": "Number of radial points",
                                           'tip': 'Too few points may lead to huge rounding errors.'}
        self._params_dict["radial_unit"] = {"type": "enum", "value": IntegrateRadial_RadialUnit.unit1,
                                            "text": "Radial Range Unit"}
        self._params_dict["radial_range"] = {"type": "tuple_float", "value": None, "text": "Radial Range",
                                             "tip": "(Inner Limit,Outer Limit).Unit is same as 'Radial Range Unit'.If not provided, range is simply (min, max)."}
        self._params_dict["polarization_factor"] = {"type": "float", "value": None, "text": "Polarization Factor",
    ...
    def run_function(self, data, objectset):
        ...
        result1d = bf.integrateRadial(objectset['integrator'], data['image'], npt=self.get_param_value("npt"),
                                      npt_rad=self.get_param_value("npt_radial"), mask=mask,
        ...
        return {'data': {'x': result1d.radial, 'y': result1d.intensity},
                'label': {'xlabel': self.get_param_value("out_unit"), 'ylabel': 'Intensity'},
                'plot': {'data': {'x': result1d.radial, 'y': result1d.intensity},
                         'type': '1DP',
                         'label': {'xlabel': self.get_param_value("out_unit").value, 'ylabel': 'Intensity'}}
                }
...
class IntegrateRadial_BgDel(ProcessingFunction):
    function_text = "Radial Integration & Background Subtraction"
    function_tip = "2D image --> 1D curve I~chi(degree/rad)"

    def __init__(self):
        super().__init__()
        self._params_dict["npt"] = {"type": "int", "value": None, "text": "Number of output points"}
        self._params_dict["out_unit"] = {"type": "enum", "value": IntegrateRadial_BgDel_OutUnit.unit1,
                                         "text": "Output Unit"}
        self._params_dict["azimuth_range"] = {"type": "tuple_float", "value": None, "text": "Azimuth Range",
                                              "tip": "(Lower angle, upper angle) in degree. Angle must between -180 and 180. 0 degree is in the direction of the left horizontal line. [Clockwise]. If not provided, range is simply (min, max)."}
        self._params_dict["npt_radial"] = {"type": "int", "value": None, "text": "Number of radial points",
                                           'tip': 'Too few points may lead to huge rounding errors.'}
        self._params_dict["radial_unit"] = {"type": "enum", "value": IntegrateRadial_BgDel_RadialUnit.unit1,
                                            "text": "Radial Range Unit"}
        self._params_dict["radial_range"] = {"type": "tuple_float", "value": None, "text": "Radial Range",
                                             "tip": "(Inner Limit,Outer Limit).Unit is same as 'Radial Range Unit'."}
        self._params_dict["polarization_factor"] = {"type": "float", "value": None, "text": "Polarization Factor",
                                                    "tip": "-1 (vertical) ~ +1 (horizontal). Default[None] is no correction, 0 for circular polarization or random."}
        self._params_dict["radial_range_low"] = {"type": "tuple_float", "value": None,
                                                 "text": "Lower Range for Background",
                                                 "tip": "(Inner Limit,Outer Limit).Unit is same as 'Radial Range Unit'."}
        self._params_dict["radial_range_high"] = {"type": "tuple_float", "value": None,
                                                  "text": "Higher Range for Background",
                                                  "tip": "(Inner Limit,Outer Limit).Unit is same as 'Radial Range Unit'."}
    ...
    def run_function(self, data, objectset):
        ...
        resultBgHigh = bf.integrateRadial(objectset['integrator'], data['image'], mask=mask,
                                          ...
                                          radial_range=self.get_param_value("radial_range_high"),
                                          polarization_factor=self.get_param_value("polarization_factor")
                                          )
