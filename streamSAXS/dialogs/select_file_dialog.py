...
class Select_file_dialog(QDialog):
    ...
    def hdf5_box(self):
        ...
        self.h5dataset_select = QLineEdit()
        widget2 = self._set_widget_layout("h5 dataset", self.h5dataset_select, self.select_h5dataset)
    ...
    def select_h5dataset(self):
        ...
        if file:
            try:
                ...
                if ok and item:
                    self.file_info["h5_dataset"] = str(item)
                    self.h5dataset_select.setText(str(item))
                f.close()
            except:
                print("No dataset")
